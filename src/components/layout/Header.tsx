...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 w-full max-w-full border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-xl px-3 sm:px-4 lg:px-8 py-2.5 flex items-center justify-between overflow-hidden">
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <div className="md:hidden flex items-center gap-1.5 shrink-0">
          ...
          <span className="font-bold text-white text-xs hidden xs:inline">TradeJournal</span>
        </div>
        ...
        <div className="relative min-w-0">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-medium text-slate-200 hover:border-slate-700 transition max-w-[160px] sm:max-w-[220px]"
          >
            <span className="w-1.5 h-1.5 rounded-full bg-brand-500 shrink-0" />
            <span className="font-semibold text-white truncate text-xs">
              {activePortfolio?.name || 'พอร์ตหลัก'}
            </span>
            <span className="text-[10px] text-slate-400 font-mono bg-slate-800 px-1 py-0.5 rounded shrink-0">
